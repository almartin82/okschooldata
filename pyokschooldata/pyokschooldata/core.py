"""
Core functions wrapping okschooldata R package via rpy2.
"""
...
def _get_pkg():
    ...
    global _pkg
    if _pkg is None:
        _pkg = importr("okschooldata")
    return _pkg


def fetch_enr(end_year: int) -> pd.DataFrame:
    """
    Fetch Oklahoma school enrollment data for a single year.
...
def fetch_enr_multi(end_years: list[int]) -> pd.DataFrame:
    """
    Fetch Oklahoma school enrollment data for multiple years.
    ...
    --------
    >>> import pyokschooldata as ok
    >>> df = ok.fetch_enr_multi([2020, 2021, 2022])
    """
    pkg = _get_pkg()
