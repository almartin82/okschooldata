# ==============================================================================
# LIVE Pipeline Tests for okschooldata
# ==============================================================================
#
...
test_that("Oklahoma DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Oklahoma DOE URL and verify HTTP 200")
})
...
test_that("Can download Oklahoma enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Oklahoma enrollment file", {
  skip_if_offline()
...
test_that("okschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- okschooldata::get_available_years()
  ...
  tryCatch({
    raw <- okschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- okschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- okschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- okschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- okschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- okschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- okschooldata:::get_cache_path(2024, "enrollment")
