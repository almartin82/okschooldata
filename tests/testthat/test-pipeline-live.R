...
test_that("Alaska DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Alaska DOE URL and verify HTTP 200")
})
...
test_that("Can download Alaska enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Alaska enrollment file", {
  skip_if_offline()
...
test_that("akschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- akschooldata::get_available_years()
  ...
  tryCatch({
    raw <- akschooldata:::get_raw_enr(test_year)
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- akschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- akschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- akschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- akschooldata::fetch_enr(test_year, tidy = TRUE)
