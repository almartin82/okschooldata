...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pyakschooldata as ak
        assert hasattr(ak, 'fetch_enr')
        assert hasattr(ak, 'fetch_enr_multi')
        assert hasattr(ak, 'tidy_enr')
        assert hasattr(ak, 'get_available_years')

    def test_version_exists(self):
        """Package has a version string."""
        import pyakschooldata as ak
        assert hasattr(ak, '__version__')
        assert isinstance(ak.__version__, str)
...
class TestGetAvailableYears:
    ...
    def test_reasonable_year_range(self):
        """Years are in a reasonable range for Alaska data."""
        import pyakschooldata as ak
        years = ak.get_available_years()
        assert years['min_year'] >= 2015
        assert years['min_year'] <= 2020
        assert years['max_year'] >= 2024
    ...
    def test_total_enrollment_reasonable(self):
        ...
        if 'is_state' in df.columns and 'subgroup' in df.columns and 'grade_level' in df.columns:
            ...
            if len(total_df) > 0:
                ...
                # Alaska should have ~130k students (small state)
